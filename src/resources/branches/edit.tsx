...
export const BranchEdit = () => {
  ...
  // Effect to parse initial class_timings and populate dropdowns
  useEffect(() => {
    const initialClassTimings = watch("class_timings");
    if (initialClassTimings && typeof initialClassTimings === 'string') {
      const parts = initialClassTimings.split(" - ");
      if (parts.length === 2) {
        const [startTime, endTime] = parts;
        const [sTime, sAmPm] = startTime.split(" ");
        const [sHour, sMinute] = sTime.split(":");
        const [eTime, eAmPm] = endTime.split(" ");
        const [eHour, eMinute] = eTime.split(":");
  ...
  }, [watch("class_timings")]); // Depend on the form's class_timings value
